...
- **AI & Analytics Utilities** – Serverless functions for topic recommendations, DuckDB-powered semantic search, tech-trend aggregation, and CMS helpers.
...
| **Serverless** | `api/*.js` (Vercel/Netlify compatible) for conversions, DuckDB semantic search, feeds, journaling |
...
└── src/data/semantic.duckdb  # DuckDB vector store generated by scripts/index-duckdb.mjs
```
...
The semantic search API reads vectors from `src/data/semantic.duckdb`. Regenerate it whenever blog content changes:

```bash
npm run --workspace apps/personal-website semantic:index
```

The script loads every `src/pages/blog/*.jsx` file, strips JSX to text, calls Gemini `text-embedding-004`, and upserts the embeddings into the DuckDB store. Production builds run the same command via `npm run build`, so keeping `GEMINI_API_KEY` set ensures Vercel/Netlify builds stay up to date.
...
`npm run dev` uses the custom Vite middleware to proxy `/api/convert` and `/api/semantic-search` into the Node handlers so DuckDB queries work without deploying functions.
