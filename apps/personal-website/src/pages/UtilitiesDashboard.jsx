...
function normalizedGasUsdPerKwh(gasItem) {
  ...
  const energyPerUnitKwh =
    gasItem.unitLabel === 'm³'
      ? 9.94
      : gasItem.unitLabel === 'kg'
        ? 12.78
        : gasItem.unitLabel === 'kWh-e'
          ? 1.0
          : null;

  const localUnitPrice = gasItem.costPerUnitLocal ?? gasItem.costPerM3;
  if (!energyPerUnitKwh || localUnitPrice == null) {
    return null;
  }
...
export default function UtilitiesDashboard() {
  ...
  const _maxM3 = Math.max(...m3Items.map((d) => d.unitsM3 || 0), 0);
  const maxUnitsLocal = Math.max(...DATA.gas.map((d) => d.unitsLocal || 0), 0);
  const gasLocalUnitPrices = DATA.gas.map(
    (d) => d.costPerUnitLocal ?? d.costPerM3
  );
  ...
  const normalizedGas = DATA.gas.map((d) => ({
    city: normalizeCity(d.city),
    value: normalizedGasUsdPerKwh(d),
  }));
  const maxNormalizedUsdPerKwh = Math.max(
    ...normalizedGas.map((d) => d.value || 0)
  );
  ...
  function addToCity(city, key, currency, value) {
    ...
    if (!cityTotals.has(normalized)) {
      cityTotals.set(normalized, {
        electricity: 0,
        gas: 0,
        water: 0,
        total: 0,
    ...
    >
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-blue-700">
              <ul className="list-disc list-inside space-y-1">
                <li>
                  Charts compare usage and costs across cities (bars scaled
                  within each category)
                </li>
                <li>
                  All costs converted to USD for apples-to-apples comparison
                </li>
                <li>Hover over stacked bars for detailed breakdown</li>
                <li>
                  Notes provide context for unusual values or missing data
                </li>
              </ul>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-indigo-600">
                  {DATA.electricity
                    .reduce((max, d) => (d.unitsKwh > max.unitsKwh ? d : max))
                    .unitsKwh.toLocaleString()}{' '}
                  kWh
                </p>
                <p className="text-sm text-indigo-600">
                  {
                    DATA.electricity.reduce((max, d) =>
                      d.unitsKwh > max.unitsKwh ? d : max
                    ).city
                  }
                </p>
              </div>
                ...
                <p className="text-2xl font-bold text-amber-600">
                  ...
                  {Math.min(
                    ...DATA.electricity.map(
                      (d) =>
                        toUsdEquivalent(d.currency, d.costPerKwh) || Infinity
                    )
                  ).toFixed(3)}
                </p>
                <p className="text-sm text-amber-600">
                  {
                    DATA.electricity.reduce((min, d) => {
                      const usdCost =
                        toUsdEquivalent(d.currency, d.costPerKwh) || Infinity;
                      const minUsdCost =
                        toUsdEquivalent(min.currency, min.costPerKwh) ||
                        Infinity;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-cyan-600">
                  {
                    new Set([
                      ...DATA.electricity.map((d) => normalizeCity(d.city)),
                      ...DATA.gas.map((d) => normalizeCity(d.city)),
                      ...DATA.water.map((d) => normalizeCity(d.city)),
                    ]).size
                  }
                </p>
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold mb-2">
          Electricity — kWh by City
        </h2>
        <p className="text-gray-600 mb-4">
          Compares absolute electricity consumption across cities.
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="p-4 bg-white rounded-xl shadow">
            <h3 className="font-medium mb-3">
              Cost per kWh (bars in USD; label local + USD)
            </h3>
            {DATA.electricity.map((d) => (
              <BarRow
                key={`${d.city}-costkwh`}
        ...
        <div className="mt-2 p-3 bg-amber-50 border-l-4 border-amber-400 rounded-r">
          <div className="text-sm text-amber-800">
            <strong>Note:</strong> Altamonte Springs, FL is a large
            bungalow-style house with a mini pool. Pool maintenance costs
            (filtering, cleaning, chemical treatment) are not included in
            utility bills but contribute to overall household expenses. Pool
            pumps typically run 6-12 hours daily, adding to electricity
            consumption.
          </div>
        </div>
        <div className="mt-2 p-3 bg-blue-50 border-l-4 border-blue-400 rounded-r">
          <div className="text-sm text-blue-800">
            <strong>Note:</strong> Coimbatore, TN uses a Reverse Osmosis (RO)
            system for groundwater treatment across all usage types (drinking,
            cooking, bathing, etc.). This advanced water purification system
            significantly increases water costs compared to typical Indian
            household costs, as RO systems require electricity for operation,
            regular filter replacements, and maintenance.
          </div>
        </div>
      </section>
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-4">
          Consumption in m³ and unit prices (bars in USD; label local + USD).
        </p>
        <div className="grid md:grid-cols-2 gap-6">
          <div className="p-4 bg-white rounded-xl shadow">
          ...
          <div className="p-4 bg-white rounded-xl shadow">
            <h3 className="font-medium mb-3">
              Cost per m³ (bars in USD; label local + USD)
            </h3>
            {DATA.water.map((d) => (
              <BarRow
      ...
      <section className="mb-10">
        ...
        <p className="text-gray-600 mb-4">
          Bars show local units consumed and local/US unit prices.
        </p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="p-4 bg-white rounded-xl shadow">
            ...
            {DATA.gas.map((d) => (
              <BarRow
                ...
                max={maxUnitsLocal || 1}
                rightLabel={
                  d.unitsLocal != null && d.unitLabel
                    ? `${d.unitsLocal} ${d.unitLabel}`
                    : '—'
                }
          ...
          <div className="p-4 bg-white rounded-xl shadow">
            <h3 className="font-medium mb-3">
              Cost per unit (bars in USD; label local + USD)
            </h3>
            {DATA.gas.map((d) => (
              <BarRow
                key={`${d.city}-costunit`}
                ...
                value={(() => {
                  ...
                  return local != null
                    ? toUsdEquivalent(d.currency, local) || 0
                    : 0;
                })()}
                ...
                rightLabel={(() => {
                  ...
                  const unit =
                    d.unitLabel || (d.costPerM3 != null ? 'm³' : 'unit');
        ...
        <div className="mt-4 text-sm text-gray-600 space-y-1">
          {DATA.gas.map((d) => (
            <div key={`${d.city}-gasnote`}>
              • {d.city}: {d.label}
              {d.total != null
                ? ` — Total: ${formatCurrency(d.total, d.currency)}`
                : ''}
            </div>
          ))}
        </div>
        <div className="mt-2 text-sm text-gray-600">
          Note: Altamonte Springs, FL (Orlando area) hotplate is electric; no
          gas usage.
        </div>
      </section>
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold mb-2">
          Normalized Gas Cost (USD/kWh)
        </h2>
        <p className="text-gray-600 mb-4">
          Energy-normalized comparison using LHV: natural gas ≈ 9.94 kWh/m³, LPG
          ≈ 12.78 kWh/kg, City Energy (SG) shown as billed kWh-e.
        </p>
        <div className="p-4 bg-white rounded-xl shadow">
          ...
          {normalizedGas.map((d) => (
            <BarRow
              ...
              rightLabel={
                d.value != null
                  ? `$${d.value.toLocaleString(undefined, { maximumFractionDigits: 3 })}/kWh`
                  : '—'
              }
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold mb-2">
          Hotplate Electricity Consumption
        </h2>
        <p className="text-gray-600 mb-4">
          4-burner electric range consumption compared to actual monthly bills
          from our utility data.
        </p>
        ...
        <div className="p-4 bg-white rounded-xl shadow">
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div>
              <h3 className="font-medium mb-3">
                Daily Consumption (2 hours cooking)
              </h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
          ...
          <div className="mt-4 p-3 bg-green-50 rounded-lg">
            <h4 className="font-medium mb-2 text-green-800">
              Comparison with Actual Bills (Electric Cooking Cities):
            </h4>
            <div className="text-sm text-green-700 space-y-2">
              <div className="flex justify-between">
                <span>Duke Energy (Florida):</span>
                <span className="font-medium">
                  1,564 kWh total → 300 kWh = 19% of bill
                </span>
              </div>
              <div className="flex justify-between">
                <span>Toronto CARMA:</span>
                <span className="font-medium">
                  704 kWh total → 300 kWh = 43% of bill
                </span>
              </div>
            </div>
            <div className="mt-2 text-xs text-gray-600">
              Note: Singapore and Coimbatore use gas cooking (not electric
              hotplates)
            </div>
          </div>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            ...
            <ul className="text-sm text-blue-700 space-y-1">
              <li>
                • <strong>Bill Impact:</strong> Electric cooking represents
                19-43% of total electricity consumption in electric cooking
                cities
              </li>
              <li>
                • <strong>Regional Variation:</strong> Higher impact in Toronto
                (43%) vs Florida (19%) due to lower overall consumption
              </li>
              <li>
                • <strong>Peak Usage Impact:</strong> Cooking during peak hours
                (5-8 PM) can significantly impact electricity bills in
                time-of-use areas
              </li>
              <li>
                • <strong>Cost Impact:</strong> At local rates: Florida
                $55.80/month, Toronto C$36.60/month
              </li>
            </ul>
          </div>
        </div>
      ...
      <section className="mb-4">
        ...
        <ul className="list-disc list-inside text-gray-700 space-y-1">
          <li>
            Coimbatore has low cost per kWh (INR ₹2.52/kWh) with modest usage
            (323 kWh).
          </li>
          <li>
            Duke Energy (Florida) shows higher absolute usage (1,564 kWh) and a
            moderate unit cost (~$0.186/kWh).
          </li>
          <li>
            Singapore: 166 kWh at S$0.2747/kWh from SP Services breakdown; City
            Energy gas shown as 67 (bill kWh-equiv), m³ not provided.
          </li>
          <li>
            Gas data: Etobicoke recorded 21 m³ with an effective unit cost
            around C$1.78/m³ (tax-incl.). Coimbatore's LPG shows a flat monthly
            fee without units.
          </li>
          <li>
            <strong>Hotplate Impact:</strong> 4-burner electric ranges (2
            hours/day) add ~300 kWh/month, representing 19-43% of bills in
            electric cooking cities (Florida, Toronto).
          </li>
        </ul>
      </section>
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-semibold mb-2">
          All Utilities — Stacked Charges by City (USD)
        </h2>
        <p className="text-gray-600 mb-4">
          Each bar stacks Electricity + Gas + Water totals, converted to USD
          using FX (₹83.5, S$1.35, C$1.36 per USD). Hover over bars for detailed
          breakdown.
        </p>
        <div className="p-4 bg-white rounded-xl shadow">
          <div className="flex items-end gap-6 overflow-x-auto pb-2">
            {stackedData.map((d) => {
              ...
              const height =
                maxStacked > 0
                  ? Math.max(6, Math.round((total / maxStacked) * 220))
                  : 0;
              ...
              const _ePercent = total > 0 ? Math.round((e / total) * 100) : 0;
              const _gPercent = total > 0 ? Math.round((g / total) * 100) : 0;
              const _wPercent = total > 0 ? Math.round((w / total) * 100) : 0;

              return (
                <div
                  key={`stack-${d.city}`}
                  className="flex flex-col items-center text-sm group relative"
                >
                  <div
                    className="w-10 bg-gray-200 rounded flex flex-col justify-end"
                    style={{ height }}
                  >
                    {e > 0 && (
                      <div
          ...
          <div className="flex items-start">
            ...
            <div className="ml-4">
              <h3 className="text-lg font-semibold text-blue-800 mb-2">
                Toronto's Futuristic Time-of-Use Pricing
              </h3>
              <div className="text-blue-700 space-y-3">
                <p>
                  <strong>Three-Tier Pricing Structure:</strong> Toronto's CARMA
                  utility implements sophisticated time-of-use pricing with
                  three distinct rate periods:
                </p>
                ...
                <p className="mt-4">
                  <strong>Granular Consumption Monitoring:</strong> This pricing
                  model suggests Toronto utilities are collecting consumption
                  data at very granular intervals—likely 15-30 minute
                  intervals—enabling precise time-of-use billing. This
                  represents a <strong>futuristic approach</strong> to utility
                  management that:
                </p>
                <ul className="list-disc list-inside space-y-1 ml-4">
                  <li>
                    Encourages demand-side management during off-peak hours
                  </li>
                  <li>
                    Provides consumers with detailed insights into their usage
                    patterns
                  </li>
                  <li>
                    Enables dynamic pricing based on real-time grid conditions
                  </li>
                  <li>
                    Supports smart grid infrastructure and renewable energy
                    integration
                  </li>
                </ul>
                <p className="mt-3 text-sm italic">
                  <strong>Note:</strong> This level of granular data collection
                  and dynamic pricing represents the future of utility
                  management, where consumers can optimize their usage patterns
