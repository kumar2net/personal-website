...
- Backend: `apps/personal-website/api/blog-tts.js` streams OpenAI Audio Speech as `audio/ogg` (opus). Caches the full buffer after streaming.
- Frontend: `BlogAudioPlayer` streams via MediaSource when supported; falls back to blob if not.
