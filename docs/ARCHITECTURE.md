...
- Vite config (`apps/personal-website/vite.config.js`) adds a custom middleware that proxies `/api/convert` requests to the local `api/convert.js` handler so unit conversions work without deploying functions.
...
- **Semantic Search**: `src/components/SemanticSearch.jsx` POSTs to `import.meta.env.VITE_SEMANTIC_SEARCH_ENDPOINT` (defaults to `/api/semantic-search`) and merges semantic + lexical results with latency metadata.
...
- Static blog metadata, embeddings, GA4 seed data, and comments live in `src/data/*.js|json`; helper utilities (`src/utils/*`) normalize dates, map recommendations, and call GNN APIs.
- Scripts under `apps/personal-website/scripts` generate sitemaps, convert docs to Markdown/JSX, collect GA4 insights, fetch Netlify comments, and run custom tests (`test-viewport`, `test-sitemap`, `test-ga4-connection`).
...
- `semantic-search.js`: Loads `src/data/semantic-mapping.json` + `semantic-embeddings.json`, optionally embeds queries via Gemini (`GEMINI_API_KEY`) and hits Vertex Vector Search IDs from env. It merges cosine scores with lexical fallbacks and returns response timings.
...
- Express backend caches analytics responses using NodeCache; semantic search caches embeddings inside serverless runtime; TL;DR data is cached in `localStorage` keyed by SHA-256 of article text; chronicled reflections persist in memory and Vercel KV when configured.
