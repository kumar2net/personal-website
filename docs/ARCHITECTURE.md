...
- Vite config (`apps/personal-website/vite.config.js`) adds a custom middleware that proxies `/api/convert` and `/api/semantic-search` requests to the real serverless handlers (loading `.env` from the repo root) so dev builds keep the same conversion + DuckDB search code paths used in production.
...
- **Semantic Search**: `src/components/SemanticSearch.jsx` POSTs to `import.meta.env.VITE_SEMANTIC_SEARCH_ENDPOINT` (defaults to `/api/semantic-search`), which generates query embeddings via Gemini and ranks documents directly out of the DuckDB vector store, returning scores + latency metadata.
...
- Static blog metadata, the DuckDB vector store (`src/data/semantic.duckdb`), GA4 seed data, and comments live under `src/data/`; helper utilities (`src/utils/*`) normalize dates, map recommendations, and call GNN APIs.
- Scripts under `apps/personal-website/scripts` generate sitemaps, convert docs to Markdown/JSX, collect GA4 insights, fetch Netlify comments, rebuild the DuckDB semantic index (`index-duckdb.mjs`), and run custom tests (`test-viewport`, `test-sitemap`, `test-ga4-connection`).
...
- `semantic-search.js`: Opens `src/data/semantic.duckdb` read-only, embeds queries via Gemini (`GEMINI_API_KEY`), computes cosine similarity in DuckDB SQL, and returns the highest-scoring posts plus timing metadata.
...
- Express backend caches analytics responses using NodeCache; semantic search reads vectors from the DuckDB file (so only Gemini query embeddings are computed at runtime); TL;DR data is cached in `localStorage` keyed by SHA-256 of article text; chronicled reflections persist in memory and Vercel KV when configured.
...
graph LR
  ...
  subgraph Data
    SemanticDuckDB[src/data/semantic.duckdb]
