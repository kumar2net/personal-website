...
The manifest auto-loads `tokens.json`, `slots.json`, and `metadata.json` through `template.companionFiles`, so the repo CLI sees one coherent render bundle while you still edit the companion files directly.
