...
function testViewportConfiguration() {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    logTestResult('Viewport meta tag present', hasViewportMeta ? 'PASS' : 'FAIL');
    
    // Test proper viewport configuration
    const hasProperViewport = content.includes('width=device-width, initial-scale=1.0');
    logTestResult('Proper viewport configuration', hasProperViewport ? 'PASS' : 'FAIL');
    ...
    const hasAppleMobileWebApp = content.includes('apple-mobile-web-app-capable');
    logTestResult('Apple mobile web app capable', hasAppleMobileWebApp ? 'PASS' : 'FAIL');
    
    // Test viewport-fit for notched devices
    const hasViewportFit = content.includes('viewport-fit=cover');
    logTestResult('Viewport fit for notched devices', hasViewportFit ? 'PASS' : 'SKIP');
...
function testMobileCSS() {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    logTestResult('Mobile-specific CSS present', hasMobileStyles ? 'PASS' : 'FAIL');
    
    // Test overflow-x hidden
    const hasOverflowHidden = content.includes('overflow-x: hidden');
    logTestResult('Horizontal overflow hidden', hasOverflowHidden ? 'PASS' : 'FAIL');
    
    // Test webkit transforms for performance
    const hasWebkitTransforms = content.includes('-webkit-transform');
    logTestResult('Webkit transforms for performance', hasWebkitTransforms ? 'PASS' : 'FAIL');
    
    // Test backface visibility
    const hasBackfaceVisibility = content.includes('backface-visibility');
    logTestResult('Backface visibility hidden', hasBackfaceVisibility ? 'PASS' : 'FAIL');
  }
}
...
function testScrollToTop() {
  ...
  if (fs.existsSync(scrollToTopPath)) {
    ...
    logTestResult('Scroll to top functionality', hasScrollToTop ? 'PASS' : 'FAIL');
    ...
    logTestResult('Location change detection', hasUseLocation ? 'PASS' : 'FAIL');
    
    // Test hash preservation
    const hasHashCheck = content.includes('hash');
    logTestResult('Hash preservation', hasHashCheck ? 'PASS' : 'FAIL');
  } else {
    logTestResult('ScrollToTop component exists', 'FAIL', 'Component not found');
  }
}
...
function testAppMobileHandling() {
  ...
  if (fs.existsSync(appPath)) {
    ...
    logTestResult('ScrollToTop imported', hasScrollToTopImport ? 'PASS' : 'FAIL');
    
    // Test ScrollToTop usage
    const hasScrollToTopUsage = content.includes('<ScrollToTop />');
    logTestResult('ScrollToTop used in App', hasScrollToTopUsage ? 'PASS' : 'FAIL');
    
    // Test mobile menu state
    const hasMobileMenuState = content.includes('isMobileMenuOpen');
    logTestResult('Mobile menu state management', hasMobileMenuState ? 'PASS' : 'FAIL');
    
    // Test mobile menu toggle
    const hasMobileMenuToggle = content.includes('setIsMobileMenuOpen');
    logTestResult('Mobile menu toggle functionality', hasMobileMenuToggle ? 'PASS' : 'FAIL');
  } else {
...
function testResponsiveDesign() {
  ...
  if (fs.existsSync(appPath)) {
    ...
    logTestResult('Responsive breakpoint classes', hasResponsiveClasses ? 'PASS' : 'FAIL');
    
    // Test mobile-specific classes
    const hasMobileClasses = content.includes('mobile') || content.includes('sm:');
    logTestResult('Mobile-specific classes', hasMobileClasses ? 'PASS' : 'FAIL');
...
function testMainJSXMobileHandling() {
  ...
  if (fs.existsSync(mainPath)) {
    ...
    logTestResult('DOMContentLoaded listener', hasDOMContentLoaded ? 'PASS' : 'FAIL');
    
    // Test error handling
    const hasErrorHandling = content.includes('try') && content.includes('catch');
    logTestResult('Error handling in main.jsx', hasErrorHandling ? 'PASS' : 'FAIL');
    
    // Test root element check
    const hasRootElementCheck = content.includes('getElementById(\'root\')');
    logTestResult('Root element existence check', hasRootElementCheck ? 'PASS' : 'FAIL');
  } else {
...
function testBlogPostMobileResponsiveness() {
  ...
  if (fs.existsSync(blogDir)) {
    ...
    blogFiles.forEach(file => {
      ...
      if (content.includes('max-w-') || content.includes('px-4') || content.includes('py-')) {
        postsWithMobileOptimization++;
      }
    });
    
    const responsiveRatio = totalPosts > 0 ? postsWithResponsiveClasses / totalPosts : 0;
    logTestResult('Blog posts with responsive classes', 
      responsiveRatio >= 0.8 ? 'PASS' : 'FAIL',
      `${postsWithResponsiveClasses}/${totalPosts} posts (${Math.round(responsiveRatio * 100)}%)`);
    
    const mobileOptimizationRatio = totalPosts > 0 ? postsWithMobileOptimization / totalPosts : 0;
    logTestResult('Blog posts with mobile optimization', 
...
function testCSSOutput() {
  ...
  if (fs.existsSync(outputCSSPath)) {
    ...
    const hasResponsiveUtilities = content.includes('@media') || content.includes('sm:') || content.includes('md:');
    logTestResult('Responsive utilities in CSS', hasResponsiveUtilities ? 'PASS' : 'FAIL');
    
    // Test mobile-first approach
    const hasMobileFirst = content.includes('@media (min-width:') || content.includes('@media (max-width:');
    logTestResult('Mobile-first media queries', hasMobileFirst ? 'PASS' : 'SKIP');
    ...
    logTestResult('output.css exists', 'SKIP', 'CSS file not found (may need build)');
  }
}
