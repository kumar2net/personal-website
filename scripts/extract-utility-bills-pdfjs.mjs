...
async function extractUtilityBillData() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (file.endsWith('.pdf')) {
        try {
          ...
          console.log(`✅ Extracted ${pdfDocument.numPages} pages from ${file}`)
        } catch (error) {
    ...
    // Save extracted data
    const outputPath = path.join(projectRoot, 'docs', 'utildata', 'extracted_data.json')
    ...
    results.forEach(result => {
      ...
      if (result.content && result.content.length > 0 && result.content !== 'Image file - requires OCR for text extraction') {
