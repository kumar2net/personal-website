...
class WordPressCrossPoster {
  ...
  async getValidToken() {
    ...
    console.log(
      '❌ No valid token available. Please run: npm run wordpress:token'
    );
    return null;
  }
  ...
  async extractBlogContent(jsxFilePath) {
    try {
      ...
      const tags = badgeMatches
        ? badgeMatches.map((tag) => tag.match(/>([^<]+)</)[1])
        : [];
      ...
      if (!contentMatch) {
        ...
        if (h1Match) {
          const afterH1 = content.substring(
  ...
  convertJSXToHTML(jsxContent) {
    const html = jsxContent
      ...
      .replace(
        /<img[^>]*src="\/media\/([^"]*)"[^>]*>/g,
        '<img src="https://kumar2net.files.wordpress.com/$1" alt="$1" />'
      )
  ...
  async generateTldrLocally(text, _title) {
    try {
  ...
  async crossPostBlog(jsxFilePath) {
    try {
      ...
      const plainText = postData.content
        ...
        .replace(/\s+/g, ' ')
        .trim();
      const tldrHtml = await this.generateTldrLocally(
        plainText,
        postData.title
      );
...
if (process.argv[2] === '--all') {
  ...
  console.log('Usage:');
  console.log(
    '  node scripts/wordpress-crosspost.mjs --all                    # Cross-post all blogs'
  );
  console.log(
    '  node scripts/wordpress-crosspost.mjs --latest                 # Cross-post latest blog'
  );
  console.log(
    '  node scripts/wordpress-crosspost.mjs path/to/blog.jsx        # Cross-post specific blog'
  );
  console.log('');
  ...
  console.log(
    '  2. Set environment variable: export WORDPRESS_API_TOKEN="your_token"'
  );
