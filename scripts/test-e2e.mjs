...
function testCriticalUserFlows() {
  ...
  logTestResult(
    'Home page loads',
    homePageTest.success && homePageTest.output.includes('200')
      ? 'PASS'
      : 'FAIL',
  ...
  const blogPageTest = runCommand(
    'curl -s -o /dev/null -w "%{http_code}" http://localhost:5173/blog',
    'Blog page accessibility'
  );
  logTestResult(
    'Blog page loads',
    blogPageTest.success && blogPageTest.output.includes('200')
      ? 'PASS'
      : 'FAIL',
  ...
  const contactPageTest = runCommand(
    'curl -s -o /dev/null -w "%{http_code}" http://localhost:5173/contact',
    'Contact page accessibility'
  );
  logTestResult(
    'Contact page loads',
    contactPageTest.success && contactPageTest.output.includes('200')
      ? 'PASS'
      : 'FAIL',
    contactPageTest.success
      ? `HTTP ${contactPageTest.output}`
  ...
  const navigationTest = runCommand(
    'curl -s http://localhost:5173/ | grep -q "navigation"',
    'Navigation presence'
  );
  logTestResult(
    'Navigation works',
...
function testResponsiveDesign() {
  ...
  if (fs.existsSync(appPath)) {
    ...
    const hasMobileMenu =
      content.includes('mobile') || content.includes('hamburger');
...
function testAccessibility() {
  ...
  if (fs.existsSync(appPath)) {
    ...
    const hasSemanticHTML = /<nav|<main|<section|<article|<header|<footer/.test(
      content
    );
