...
function runUnitTests() {
  ...
  if (fs.existsSync(disqusComponentPath)) {
    ...
    logTestResult(TEST_CATEGORIES.UNIT_TESTS, 'DisqusComments component', 'FAIL', 'Component not found');
  }
  ...
  criticalComponents.forEach(componentPath => {
    if (fs.existsSync(componentPath)) {
      logTestResult(TEST_CATEGORIES.UNIT_TESTS, `${path.basename(componentPath)} exists`, 'PASS');
    } else {
      logTestResult(TEST_CATEGORIES.UNIT_TESTS, `${path.basename(componentPath)} exists`, 'FAIL', 'Component not found');
    }
  });
}
...
function runIntegrationTests() {
  ...
  if (fs.existsSync(blogDir)) {
    ...
    logTestResult(TEST_CATEGORIES.INTEGRATION_TESTS, 'Blog posts with Disqus', 
      postsWithDisqus === totalPosts ? 'PASS' : 'FAIL', 
      `${postsWithDisqus}/${totalPosts} posts`);
    
    logTestResult(TEST_CATEGORIES.INTEGRATION_TESTS, 'Blog posts properly configured', 
  ...
  logTestResult(TEST_CATEGORIES.INTEGRATION_TESTS, 'Build process', 
...
function runE2ETests() {
  ...
  const e2eTests = [
    ...
    { name: 'Blog page loads', url: 'http://localhost:5173/blog', critical: true },
    { name: 'Contact page loads', url: 'http://localhost:5173/contact', critical: true },
    { name: 'Navigation works', url: 'http://localhost:5173/', critical: true },
    { name: 'Blog post with Disqus loads', url: 'http://localhost:5173/blog/habit', critical: true }
  ];
  ...
  e2eTests.forEach(test => {
    ...
    logTestResult(TEST_CATEGORIES.E2E_TESTS, test.name, 
...
function runAccessibilityTests() {
  ...
  const accessibilityTests = [
    ...
    { name: 'Semantic HTML', pattern: /<nav|<main|<section|<article/, required: true },
    { name: 'Keyboard navigation', pattern: /tabIndex/, required: false },
    { name: 'Color contrast', pattern: /text-gray-|text-black|text-white/, required: false }
  ];
...
function runPerformanceTests() {
  ...
  if (fs.existsSync(distPath)) {
    ...
    if (fs.existsSync(mainJsPath)) {
      const jsFiles = fs.readdirSync(mainJsPath).filter(file => file.endsWith('.js'));
      let totalSize = 0;
      ...
      logTestResult(TEST_CATEGORIES.PERFORMANCE_TESTS, 'Bundle size', 
  ...
  logTestResult(TEST_CATEGORIES.PERFORMANCE_TESTS, 'Lazy loading implemented', 
...
function runSecurityTests() {
  ...
  const securityTests = [
    { name: 'No hardcoded secrets', pattern: /password|secret|key|token/, required: false },
    { name: 'HTTPS URLs', pattern: /https:\/\//, required: true },
    { name: 'External links have rel="noopener"', pattern: /rel="noopener"/, required: false },
  ...
  componentsToTest.forEach(componentPath => {
    if (fs.existsSync(componentPath)) {
      ...
      securityTests.forEach(test => {
        ...
        logTestResult(TEST_CATEGORIES.SECURITY_TESTS, `${path.basename(componentPath)} - ${test.name}`, status);
      });
    }
  });
}
...
function runUXTests() {
  ...
  const uxTests = [
    { name: 'Loading states implemented', pattern: /loading|spinner|animate-spin/, required: true },
    { name: 'Error handling implemented', pattern: /error|catch|try/, required: true },
    { name: 'Responsive design classes', pattern: /md:|lg:|sm:|xl:/, required: true },
  ...
  componentsToTest.forEach(componentPath => {
    if (fs.existsSync(componentPath)) {
      ...
      uxTests.forEach(test => {
        const hasFeature = test.pattern.test(content);
        const status = hasFeature ? 'PASS' : (test.required ? 'FAIL' : 'SKIP');
        logTestResult(TEST_CATEGORIES.UX_TESTS, `${path.basename(componentPath)} - ${test.name}`, status);
      });
    }
  });
}
