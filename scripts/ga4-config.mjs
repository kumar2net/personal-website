...
export const GA4_CONFIG = {
  ...
  serviceAccount: {
    ...
    credentials: process.env.GCP_SERVICE_ACCOUNT_JSON ? 
      JSON.parse(process.env.GCP_SERVICE_ACCOUNT_JSON) : null
  },
...
export function getServiceAccountCredentials() {
  if (!GA4_CONFIG.serviceAccount.credentials) {
    throw new Error('GCP_SERVICE_ACCOUNT_JSON environment variable not found or invalid');
  }
...
export function printConfig() {
  ...
  console.log(`🔑 Credentials Loaded: ${GA4_CONFIG.serviceAccount.credentials ? '✅' : '❌'}`);
  console.log(`📡 API Version: ${GA4_CONFIG.api.version}`);
}

// Export individual config sections for convenience
export const { property, project, serviceAccount, api, dateRanges, metrics, dimensions, output } = GA4_CONFIG;
