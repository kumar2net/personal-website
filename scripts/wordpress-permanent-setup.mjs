...
class WordPressPermanentSetup {
  ...
  async generateInitialToken() {
    ...
    console.log('');
    console.log(
      'This will create a permanent solution for WordPress cross-publishing.'
    );
    console.log(
      'You only need to do this ONCE, and the system will automatically'
    );
    ...
    console.log('');
    console.log(
      `https://public-api.wordpress.com/oauth2/authorize?client_id=${this.clientId}&redirect_uri=${encodeURIComponent(this.redirectUri)}&response_type=code&scope=global`
    );
    console.log('');
    console.log(
    ...
    console.log('');
    console.log(
      'Step 3: Copy the authorization code from the URL and paste it below.'
    );
    console.log('');
    ...
    try {
      ...
      console.log(
        '🔄 The system will now automatically refresh this token when needed.'
      );
      console.log('');
      console.log('Next steps:');
      console.log(
        '1. The token is saved locally and will be used automatically'
      );
      console.log(
        '2. When the token expires (30 days), it will be refreshed automatically'
      );
      console.log(
        '3. You can test the setup with: npm run wordpress:test-token'
      );
      console.log(
        '4. Cross-publish your latest blog with: npm run crosspost:latest'
      );
    } catch (error) {
  ...
  async testSetup() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          '⚠️  Token may be expired, but refresh should work automatically.'
        );
      }
    } catch (_error) {
  ...
  async showStatus() {
    ...
    try {
      ...
      const daysRemaining = Math.max(
        0,
        Math.floor((thirtyDays - tokenAge) / (24 * 60 * 60 * 1000))
      );
      ...
      console.log(
        `🔄 Refresh token: ${tokenData.refresh_token ? 'Available' : 'Not available'}`
      );

      if (daysRemaining < 7) {
        console.log(
          '⚠️  Token will expire soon. Refresh will happen automatically.'
        );
      } else {
...
if (process.argv[2] === '--setup') {
  ...
  console.log('');
  console.log(
    'This tool provides a PERMANENT solution for WordPress cross-publishing.'
  );
  console.log('Once set up, you never need to manually generate tokens again!');
  console.log('');
  console.log('Usage:');
  console.log(
    '  npm run wordpress:permanent-setup    # Initial setup (ONE TIME ONLY)'
  );
