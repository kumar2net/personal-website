...
function generateTldrLocallyFromHtml(html) {
  ...
  const keywords = ['recommend', 'suggest', 'key', 'important', 'use', 'need', 'next', 'insight', 'summary', 'learned', 'plan', 'solution'];
  const scored = sentences.map((s, i) => ({ s, score: keywords.reduce((a, k) => a + (s.toLowerCase().includes(k) ? 1 : 0), 0) + (i < 5 ? 0.5 : 0) }));
...
async function getToken() {
  if (process.env.WORDPRESS_API_TOKEN) {
    ...
    if (await isTokenValid(t)) return t;
  }
  const fileTok = await readTokenFromFile();
  if (fileTok && (await isTokenValid(fileTok))) return fileTok;
  throw new Error('No valid WordPress API token found. Set WORDPRESS_API_TOKEN or ensure data/wordpress-token.json exists.');
}

async function main() {
  const token = await getToken();
  const postId = process.argv[2];
  if (!postId) throw new Error('Usage: node scripts/wp-add-tldr-to-post.mjs <POST_ID>');
  ...
  console.log('✅ Updated post with TL;DR:', { id: result.ID, url: result.URL });
}
