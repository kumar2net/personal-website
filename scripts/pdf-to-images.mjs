...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    const a = args[i]
    if (a === '--file') acc.file = args[++i]
    else if (a === '--out') acc.out = args[++i]
    else if (a === '--prefix') acc.prefix = args[++i]
  }
  if (!acc.file) throw new Error('Missing --file')
...
async function main() {
  ...
  console.log('✅ Saved images:', files.map(f => path.basename(f)).join(', '))
}
