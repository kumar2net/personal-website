...
const PHRASE = 'This cross-post includes a TL;DR and source link to avoid duplicate-content SEO issues.';
...
async function getToken() {
  ...
  if (process.env.WORDPRESS_API_TOKEN) {
    ...
    if (await isTokenValid(envTok)) return envTok;
  }
...
function cleanAttribution(html) {
  ...
  if (!html.includes(PHRASE)) return null; // no change
...
async function main() {
  ...
  for (const p of posts) {
    ...
    if (cleaned === null) continue; // no match
    try {
