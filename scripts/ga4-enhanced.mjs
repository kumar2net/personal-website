...
async function getEnhancedGA4Data() {
  try {
    ...
    // Process daily data
    const dailyData =
      dailyUsersResponse[0].rows?.map((row) => ({
    ...
    const topPages =
      topPagesResponse[0].rows?.map((row) => ({
        ...
        uniquePageViews: parseInt(row.metricValues[1].value, 10),
      })) || [];

    // Process traffic sources
    const trafficSources =
      trafficSourcesResponse[0].rows?.map((row) => ({
        ...
        engagementLevel:
          avgSessionDuration > 180
            ...
            : avgSessionDuration > 60
              ...
              : 'low',
        trafficVolume:
          totalUsers > 1000 ? 'high' : totalUsers > 100 ? 'medium' : 'low',
        retentionRate:
          bounceRate < 0.4
            ? 'good'
            : bounceRate < 0.6
...
function generateEnhancedTopicSuggestions(gaData) {
  ...
  if (gaData.trafficSources && gaData.trafficSources.length > 0) {
    ...
    const googleSource = gaData.trafficSources.find((s) =>
...
function generateEnhancedReport(gaData, suggestions) {
  const report = `# Enhanced GA4 Data Analysis - Actionable Blog Topics
...
${gaData.dailyData
  .map(
    (day) => `- **${day.date}:** ${day.users} users, ${day.sessions} sessions`
  )
  .join('\n')}
...
2. **Optimize for your primary traffic source** - Focus on ${gaData.trafficSources?.[0] ? gaData.trafficSources[0].source : 'your main traffic source'}
...
export {
  getEnhancedGA4Data,
  generateEnhancedTopicSuggestions,
